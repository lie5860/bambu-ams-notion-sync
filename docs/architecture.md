...
- `耗材规格`：relation 到 `耗材色卡`，用于按“材料 + 颜色”统计和分组；规格页标题只放材料，页面 icon 使用颜色生成色块。
...
- `耗材规格`：标题字段，只写材料，例如 `PLA`。
- `颜色`：保留原始 hex 色值，页面 icon 承担可视化色块。
- 用户删除必要字段时，下次运行会补回；同名字段类型冲突时，旧字段会重命名为 `字段名-temp`，再创建程序需要的字段。

这种设计避免新增难读的 `类型+颜色` 拼接文本。统计时可以在 `耗材用量明细` 上按 `耗材规格` 分组、筛选或汇总，Notion 里显示的是“材料标题 + 色块 icon”。
...
- `自定义统计`：table 视图，按 `耗材规格` 分组，显示 `任务耗材`、`耗材规格`、`用量g`、`打印任务`、`开始时间`。用户可以直接在这个视图里筛选 `开始时间` 的起止范围，Notion 会实时查询符合条件的明细行，并保留每条明细到打印任务的 relation。
- `自定义统计图`：chart 视图，按 `耗材规格` 分组，对 `用量g` 做 sum，用于快速看每种材料/颜色在当前筛选范围内用了多少克。
