...
AMS 耗材的 `颜色` 保存主色，`颜色列表` 保存 AMS 上报的全部色号，`颜色类型` 保存 `单色`、`多色` 或 `渐变`。页面 icon 由服务本地生成 64x64 PNG 并上传到 Notion：单色显示纯色色块，多色显示硬分段色条，渐变显示平滑渐变。

AMS 耗材的 `颜色别名` 来自 `颜色映射` 子数据库：如果当前主色存在用户填写的别名，则写入别名；否则写入规范化后的 hex 色号。这样 AMS 耗材和打印任务统计共享同一套颜色命名规则。
